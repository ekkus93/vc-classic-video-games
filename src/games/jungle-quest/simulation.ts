...
import { JUNGLE_QUEST_FINISH_ROOM, JUNGLE_QUEST_FINISH_X, JUNGLE_QUEST_SEALED_PASSAGE_DEPTH, JUNGLE_QUEST_START_ROOM, JUNGLE_QUEST_TOTAL_COLLECTIBLES, jungleQuestBoundaryOpen, jungleQuestEntryX, jungleQuestRoom, type JungleQuestCheckpoint, type JungleQuestRoom, type JungleQuestRoomId } from "./world.js";
...
export class JungleQuestSimulation {
  ...
  private resolveRoomTransition(events: JungleQuestSimulationEvent[]): void {
    ...
    if (x > JUNGLE_QUEST_RUN_RULES.logicalWidth + HALF_WIDTH && canGoRight) { target = room.next; x = jungleQuestEntryX("next"); }
    else if (x < -HALF_WIDTH && canGoLeft) { target = room.previous; x = jungleQuestEntryX("previous"); }
    if (target === null) {
      ...
      const maxX = canGoRight ? Infinity : JUNGLE_QUEST_RUN_RULES.logicalWidth - HALF_WIDTH - (sealedRight ? JUNGLE_QUEST_SEALED_PASSAGE_DEPTH : 0);
