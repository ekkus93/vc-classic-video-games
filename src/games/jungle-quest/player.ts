...
export interface JungleQuestPlayerState {
  ...
  readonly facing: -1 | 1;
  readonly mode: JungleQuestPlayerMode;
  readonly vineId: string | null;
  readonly vineAngleRadians: number;
  readonly vineAngularVelocity: number;
}

export interface JungleQuestPlayerInput {
  readonly horizontal: -1 | 0 | 1;
  readonly vertical: -1 | 0 | 1;
  readonly jumpPressed: boolean;
  readonly vinePressed: boolean;
}
...
export interface JungleQuestPlayerStep {
  readonly state: JungleQuestPlayerState;
  readonly events: readonly JungleQuestPlayerEvent[];
}
...
function ladderNear(room: JungleQuestRoom, position: Vector2): JungleQuestLadder | null {
  return (
    room.ladders.find(
      (ladder) =>
        Math.abs(position.x - ladder.x) <= LADDER_SNAP_DISTANCE &&
        position.y + HALF_HEIGHT >= ladder.yTop - 4 &&
        position.y + HALF_HEIGHT <= ladder.yBottom + 4,
    ) ?? null
  );
}

function vineById(room: JungleQuestRoom, id: string | null): JungleQuestVine | null {
  if (id === null) {
    return null;
  }
  return room.vines.find((vine) => vine.id === id) ?? null;
}
...
function latchableVine(room: JungleQuestRoom, position: Vector2): JungleQuestVine | null {
  return (
    room.vines.find(
      (vine) =>
        Math.hypot(position.x - vine.anchorX, position.y - (vine.anchorY + vine.length)) <=
        VINE_LATCH_DISTANCE,
    ) ?? null
  );
}
...
function standingPlatformY(room: JungleQuestRoom, position: Vector2): number | null {
  ...
  const platform = room.platforms.find(
    (candidate) =>
      position.x + HALF_WIDTH >= candidate.x1 &&
      position.x - HALF_WIDTH <= candidate.x2 &&
      Math.abs(feet - candidate.y) <= 1e-6,
  );
  return platform?.y ?? null;
}

function resolveLanding(
  room: JungleQuestRoom,
  previous: Vector2,
  next: Vector2,
  velocityY: number,
): { readonly y: number; readonly landed: boolean } {
  if (velocityY < 0) {
    return { y: next.y, landed: false };
  }
  const previousFeet = previous.y + HALF_HEIGHT;
  const nextFeet = next.y + HALF_HEIGHT;
  let landingY: number | null = null;
  for (const platform of room.platforms) {
    if (
      next.x + HALF_WIDTH < platform.x1 ||
      next.x - HALF_WIDTH > platform.x2 ||
      previousFeet > platform.y + 0.5 ||
      nextFeet < platform.y
    ) {
      continue;
    }
    if (landingY === null || platform.y < landingY) {
      landingY = platform.y;
    }
  }
...
function enterLadder(state: JungleQuestPlayerState, ladder: JungleQuestLadder): JungleQuestPlayerState {
  return Object.freeze({
    ...state,
    position: Object.freeze({ x: ladder.x, y: state.position.y }),
...
function stepLadder(
  state: JungleQuestPlayerState,
  ladder: JungleQuestLadder,
  input: JungleQuestPlayerInput,
  dtSeconds: number,
): JungleQuestPlayerState {
  const topY = ladder.yTop - HALF_HEIGHT;
  const bottomY = ladder.yBottom - HALF_HEIGHT;
  const y = clamp(
    state.position.y + input.vertical * JUNGLE_QUEST_RUN_RULES.climbSpeed * dtSeconds,
    topY,
    bottomY,
  );
  ...
  return Object.freeze({
    ...state,
    position: Object.freeze({ x: ladder.x, y }),
    velocity: Object.freeze({ x: 0, y: input.vertical * JUNGLE_QUEST_RUN_RULES.climbSpeed }),
    mode: atEnd && input.vertical !== 0 ? "ground" : "ladder",
  });
}

function stepVine(
  state: JungleQuestPlayerState,
  vine: JungleQuestVine,
  input: JungleQuestPlayerInput,
  dtSeconds: number,
): JungleQuestPlayerStep {
  ...
  angularVelocity = clamp(
    angularVelocity,
    -JUNGLE_QUEST_RUN_RULES.maxVineAngularSpeed,
    JUNGLE_QUEST_RUN_RULES.maxVineAngularSpeed,
  );
  ...
  const position = Object.freeze({
    x: vine.anchorX + Math.sin(angle) * vine.length,
    y: vine.anchorY + Math.cos(angle) * vine.length,
  });
  if (input.jumpPressed || input.vinePressed) {
    const velocity = Object.freeze({
      x: Math.cos(angle) * vine.length * angularVelocity + input.horizontal * 12,
      y: -Math.sin(angle) * vine.length * angularVelocity - 22,
    });
  ...
  return Object.freeze({
    state: Object.freeze({
      ...
      position,
      velocity: Object.freeze({ x: 0, y: 0 }),
      facing: input.horizontal === 0 ? state.facing : input.horizontal,
      mode: "vine",
      vineId: vine.id,
      vineAngleRadians: angle,
      vineAngularVelocity: angularVelocity,
    }),
    events: Object.freeze(events),
  });
}
...
export function stepJungleQuestPlayer(
  state: JungleQuestPlayerState,
  input: JungleQuestPlayerInput,
  room: JungleQuestRoom,
  dtSeconds: number,
  ...
  if (state.mode === "vine" && activeVine !== null) {
    return stepVine(state, activeVine, input, dtSeconds);
  }
  const activeLadder = ladderNear(room, state.position);
  if (state.mode === "ladder" && activeLadder !== null) {
    return Object.freeze({
      state: stepLadder(state, activeLadder, input, dtSeconds),
  ...
  if (input.vertical !== 0 && input.horizontal === 0 && activeLadder !== null) {
    const ladderState = enterLadder(state, activeLadder);
    return Object.freeze({
      state: stepLadder(ladderState, activeLadder, input, dtSeconds),
      events: Object.freeze([]),
    });
  }
  if (input.vinePressed) {
    ...
    if (vine !== null) {
      const dx = state.position.x - vine.anchorX;
      const dy = state.position.y - vine.anchorY;
      const angle = clamp(
        Math.atan2(dx, dy),
        -JUNGLE_QUEST_RUN_RULES.maxVineAngleRadians,
        JUNGLE_QUEST_RUN_RULES.maxVineAngleRadians,
      );
      const tangentX = Math.cos(angle) * vine.length;
      const tangentY = -Math.sin(angle) * vine.length;
      const tangentLengthSquared = tangentX * tangentX + tangentY * tangentY;
      const angularVelocity =
        tangentLengthSquared === 0
          ? 0
          : (state.velocity.x * tangentX + state.velocity.y * tangentY) / tangentLengthSquared;
      const position = Object.freeze({
        x: vine.anchorX + Math.sin(angle) * vine.length,
        y: vine.anchorY + Math.cos(angle) * vine.length,
      });
      return Object.freeze({
        ...
        events: Object.freeze([Object.freeze({ type: "vine-latched" as const, position })]),
      });
    }
  }
  const events: JungleQuestPlayerEvent[] = [];
  ...
  if (input.horizontal !== 0) {
    velocityX = moveToward(
      velocityX,
      input.horizontal * JUNGLE_QUEST_RUN_RULES.maxRunSpeed,
      JUNGLE_QUEST_RUN_RULES.runAcceleration * dtSeconds,
    );
  } else if (mode === "ground") {
    velocityX = moveToward(velocityX, 0, JUNGLE_QUEST_RUN_RULES.groundFriction * dtSeconds);
  }
  if (mode === "ground") {
    velocityY = 0;
    if (input.jumpPressed) {
      velocityY = -JUNGLE_QUEST_RUN_RULES.jumpSpeed;
      mode = "air";
      events.push(Object.freeze({ type: "jumped", position: state.position }));
    }
  }
  velocityY = Math.min(
    JUNGLE_QUEST_RUN_RULES.maxFallSpeed,
    velocityY + JUNGLE_QUEST_RUN_RULES.gravity * dtSeconds,
  );
  const next = Object.freeze({
    x: state.position.x + velocityX * dtSeconds,
    y: state.position.y + velocityY * dtSeconds,
  });
  ...
  return Object.freeze({
    state: Object.freeze({
      position: Object.freeze({ x: next.x, y: landing.y }),
