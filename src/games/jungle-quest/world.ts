...
export interface JungleQuestPlatform {
  readonly id: string;
  readonly x1: number;
  readonly x2: number;
  readonly y: number;
  readonly kind: JungleQuestPlatformKind;
}

export interface JungleQuestLadder {
  readonly id: string;
  readonly x: number;
...
export interface JungleQuestVine {
  readonly id: string;
...
export interface JungleQuestCollectible {
  readonly id: string;
  readonly x: number;
  readonly y: number;
...
export interface JungleQuestCheckpoint {
  readonly x: number;
  readonly y: number;
}

export interface JungleQuestRoom {
  readonly id: JungleQuestRoomId;
  readonly title: string;
  readonly previous: JungleQuestRoomId | null;
  readonly next: JungleQuestRoomId | null;
  readonly platforms: readonly JungleQuestPlatform[];
  readonly ladders: readonly JungleQuestLadder[];
  readonly vines: readonly JungleQuestVine[];
  readonly hazards: readonly Aabb[];
  readonly collectibles: readonly JungleQuestCollectible[];
  readonly checkpoint: JungleQuestCheckpoint | null;
...
function platform(
  ...
  return Object.freeze({ id, x1, x2, y, kind });
}
...
const ROOMS: readonly JungleQuestRoom[] = Object.freeze([
  Object.freeze({
    ...
    ladders: Object.freeze([
      Object.freeze({ id: "fern-ladder", x: 92, yTop: 144, yBottom: SURFACE_Y }),
    ]),
    vines: Object.freeze([]),
    hazards: Object.freeze([Object.freeze({ x: 214, y: 182, width: 24, height: 8 })]),
    collectibles: Object.freeze([
      Object.freeze({ id: "jade-seed", x: 92, y: 132, label: "Jade Seed" }),
    ]),
  ...
  Object.freeze({
    ...
    next: "root-vault",
    platforms: Object.freeze([
      platform("echo-left", 0, 116, SURFACE_Y, "surface"),
      platform("echo-right", 202, 320, SURFACE_Y, "surface"),
      platform("echo-tunnel", 0, 320, TUNNEL_Y, "tunnel"),
    ]),
    ladders: Object.freeze([
      Object.freeze({ id: "echo-descent", x: 42, yTop: SURFACE_Y, yBottom: TUNNEL_Y }),
    ]),
    vines: Object.freeze([
      Object.freeze({ id: "echo-vine", anchorX: 158, anchorY: 62, length: 118 }),
    ]),
    hazards: Object.freeze([]),
    collectibles: Object.freeze([
      Object.freeze({ id: "sun-disc", x: 252, y: 176, label: "Sun Disc" }),
    ]),
    checkpoint: null,
  ...
  Object.freeze({
    id: "root-vault",
    title: "Root Vault",
    previous: "echo-hollow",
    next: "sun-shrine",
    platforms: Object.freeze([
      platform("root-ground", 0, 320, SURFACE_Y, "surface"),
      platform("root-tunnel", 0, 320, TUNNEL_Y, "tunnel"),
    ]),
    ladders: Object.freeze([
      Object.freeze({ id: "root-ascent", x: 268, yTop: SURFACE_Y, yBottom: TUNNEL_Y }),
    ]),
    vines: Object.freeze([]),
    hazards: Object.freeze([Object.freeze({ x: 204, y: 182, width: 28, height: 8 })]),
    collectibles: Object.freeze([
      Object.freeze({ id: "root-crystal", x: 152, y: 212, label: "Root Crystal" }),
    ]),
  ...
  Object.freeze({
    ...
    previous: "root-vault",
    next: null,
    // CR2-004: the tunnel band drawn across the whole room (see TUNNEL_BAND_TOP in
    // sealed-passages.ts) used to visually suggest a passage wider than the floor actually was --
    // this platform ended at x=112 while nothing marked the drop, so walking east past the
    // shrine-ascent ladder (x=82) fell out of the world. Extended to the room's own east edge,
    // where Sun Shrine's `next: null` means there is no adjoining room to travel into, so the
    // existing world-edge clamp (resolveRoomTransition) simply holds the player on-screen there --
    // the same dead-end shape Echo Hollow's tunnel already has at its own sealed west end.
    platforms: Object.freeze([
      platform("shrine-ground", 0, 320, SURFACE_Y, "surface"),
      platform("shrine-tunnel", 0, 320, TUNNEL_Y, "tunnel"),
    ]),
    ladders: Object.freeze([
      Object.freeze({ id: "shrine-ascent", x: 82, yTop: SURFACE_Y, yBottom: TUNNEL_Y }),
    ]),
    vines: Object.freeze([]),
    hazards: Object.freeze([Object.freeze({ x: 142, y: 182, width: 22, height: 8 })]),
    collectibles: Object.freeze([
      Object.freeze({ id: "sky-amber", x: 232, y: 176, label: "Sky Amber" }),
    ]),
    checkpoint: null,
...
export const JUNGLE_QUEST_TOTAL_COLLECTIBLES = ROOMS.reduce(
  (count, room) => count + room.collectibles.length,
  0,
);

export function jungleQuestRoom(id: JungleQuestRoomId): JungleQuestRoom {
  const room = ROOM_MAP.get(id);
  if (room === undefined) {
    throw new Error(`Unknown Jungle Quest room: ${id}`);
  }
  return room;
}

export function jungleQuestCollectibleIds(): readonly string[] {
  return Object.freeze(ROOMS.flatMap((room) => room.collectibles.map((collectible) => collectible.id)));
}
...
export function jungleQuestEntryX(side: JungleQuestBoundarySide): number {
  return side === "next"
    ? HALF_PLAYER_WIDTH + 1
    : JUNGLE_QUEST_RUN_RULES.logicalWidth - HALF_PLAYER_WIDTH - 1;
}
 ...
 */
export function jungleQuestBoundaryOpen(
  room: JungleQuestRoom,
  side: JungleQuestBoundarySide,
  feetY: number,
): boolean {
  ...
  return jungleQuestRoom(neighbourId).platforms.some(
    (platform) =>
      platform.x1 <= entryX + HALF_PLAYER_WIDTH &&
      platform.x2 >= entryX - HALF_PLAYER_WIDTH &&
      platform.y >= feetY - JUNGLE_QUEST_ENTRY_SUPPORT_TOLERANCE,
  );
}

export interface JungleQuestSealedPassage {
  readonly side: JungleQuestBoundarySide;
  readonly platform: JungleQuestPlatform;
}
...
export function jungleQuestSealedPassages(room: JungleQuestRoom): readonly JungleQuestSealedPassage[] {
  ...
  for (const side of ["previous", "next"] as const) {
    ...
    for (const platform of room.platforms) {
      if (platform.x1 <= edgeX && platform.x2 >= edgeX && !jungleQuestBoundaryOpen(room, side, platform.y)) {
        sealed.push(Object.freeze({ side, platform }));
      }
    }
  }
