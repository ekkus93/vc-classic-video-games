...
export class JungleQuestEffects {
  private readonly particles = new ParticleBurstField({ maxParticles: JUNGLE_QUEST_EFFECT_RULES.maxParticles });
  private vineLoopActive = false;
  public constructor(private readonly audio: AudioService) {}
  public get particleCount(): number { return this.particles.count; }
  ...
  public update(dtSeconds: number): void { this.particles.update(dtSeconds); }
  public render(renderer: GameRenderer): void { this.particles.render(renderer); }
  public destroy(): void { this.setVineActive(false); this.particles.clear(); }
  private spawnBurst(position: Vector2, count: number, speed: number, color: string): void {
    this.particles.burst({ x: position.x, y: position.y, count, speed, lifetimeSeconds: PARTICLE_LIFETIME_SECONDS, radius: PARTICLE_RADIUS, color });
  }
}
