...
import { SPACE_ROCKS_AUDIO_IDS } from "../../games/space-rocks/effects.js";
import { SPACE_ROCKS_MODULE } from "../../games/space-rocks/module.js";
...
export const tests: readonly TestCase[] = [
  {
    name: "P7-010 Space Rocks lifecycle soak retains one RAF and releases run resources",
    run: async () => {
      ...
      for (let cycle = 0; cycle < cycles; cycle += 1) {
        await host.launch(SPACE_ROCKS_MODULE, {
          players: 1,
          difficulty: "orbit",
          seed: 0x7000 + cycle,
        });
        ...
        assert(host.activeGameId === "space-rocks", "launch must own Space Rocks");
        ...
        assert(
          services.audio.playedLoops.includes(SPACE_ROCKS_AUDIO_IDS.thrust),
          "real Space Rocks update must route thrust through shared audio",
        );
        assert(
          scheduler.pendingCount === 1,
        ...
        assert(
          ...
          "game pause must release the owned thrust loop",
        );
        ...
        await host.restart();
        assert(host.activeGameId === "space-rocks", "restart must retain game ownership");
        assert(
          scheduler.pendingCount === 1,
